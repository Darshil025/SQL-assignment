...
-- position with the first letter capitalized and all other letters lowercase, Give each column an appropriate label.
	select concat(upper(substr(last_name,2,1)),substr(last_name,3,3)) as last_name from employees; -- dought is it right?
...
--  name all in Upper case, if the Manager name consists of 'z' replace it with '$!'
	select emp from employees; -- dought can't understand question
...
-- 6. Create a query to display the last name and salary for all employees. Format the salary to be 15 characters long, left-padded with $. Label the column SALARY
	select last_name,concat("$",salary) as SALARY from employees;  -- dought about characters long

-- 7. Display the employee's name if it is a palindrome
	select concat(first_name," ",last_name) as emp_name from employees where first_name in (select reverse(first_name) from employees);		-- dought
...
-- 9. From LOCATIONS table, extract the word between first and second space from the STREET ADDRESS column.
	select street_address from locations where street_address like "% %"; -- dought
...
-- 11. Display the names and job titles of all employees with the same job as Trenna.
    ...
    where jobs.job_id = (select job_id from employees where first_name = "trenna");
...
-- 12. Display the names and department name of all employees working in the same city as Trenna.
	...
	where departments.location_id = (select locations.location_id from employees join departments on employees.department_id = departments.department_id
    join locations on departments.location_id = locations.location_id where first_name = "trenna");   
