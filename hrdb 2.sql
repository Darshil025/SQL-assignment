...
-- 8) Display the employee's name, department id who have the first name same as another employee in the same department.
	select concat(first_name," ",last_name) as employees_name,department_id from employees group by department_id,first_name having count(*) > 1 ; -- dought (clear)
...
-- 10) Select the Managers name, the count of employees working under and the department ID of the manager.
	select e1.employee_id,e1.first_name,count(*) as total_employees,e1.department_id from employees e1,employees e2
    where e1.employee_id = e2.manager_id
    group by e1.employee_id;-- dought (clear) used self-join.
